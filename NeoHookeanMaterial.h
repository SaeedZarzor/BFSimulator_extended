...
class NeoHookeanMaterial 
{
    ...
    st_case(stiffness_case), mu_cmax(shear_modulud_cortex), mu_s(shear_modulud_cortex/stiffness_ratio)
       ,nu(Poisson), c_max(max_cell_density), radial_exp(cp_radial_exp) ,R_c(subcortix_raduis),
	 ...
	 void update_material_data(const Tensor<2, dim> &F_elastic, const Point<dim> &p, const double &c)
      ...
      Tensor<4 ,dim> get_tangent_tensor_elastic() const {
          ...
          c = mu * NonStandardTensors::OverlineDyads<dim>(I,I) + cof * NonStandardTensors::underlineDyads<dim>(tr_inv_F_e,inv_F_e) +
          Lamda * outer_product(tr_inv_F_e, tr_inv_F_e);
    ...
    double get_J_e() const {return J_e;}
